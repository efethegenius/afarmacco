...
export const Deworm = () => {
  ...
  const [returnedDeworm, setReturnedDeworm] = useState([]);
  ...
  let allDeworm = returnedDeworm.name;

  const sortDeworm =
    returnedDeworm.name && search
      ? returnedDeworm.name.filter((sortedDeworm) =>
          sortedDeworm.Brand.toLowerCase().includes(search.toLowerCase())
        )
      : allDeworm;
